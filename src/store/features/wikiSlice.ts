...
import { fetchCreateTheme, fetchCreateTopic, fetchDeleteTheme, fetchDeleteTopic, fetchGetAllTopics, fetchGetThemeById, fetchGetThemesByTopic, fetchGetTopicById, fetchUpdateTheme, fetchUpdateTopic } from "@/api/server/wikiApi";

// Создание нового топика
export const createTopic = createAsyncThunk<
  Topic,
  { topic: Omit<Topic, "id" | "createdAt" | "updatedAt" | "themes">; token: string }
>(
  "wiki/createTopic",
  async ({ topic, token }) => {
    try {
...
export const getAllTopics = createAsyncThunk<Topic[], string, { rejectValue: string }>(
    "wiki/getAllTopics",
...
export const getTopicById = createAsyncThunk<Topic, { id: string; token: string }, { rejectValue: string }>(
    "wiki/getTopicById",
...
// Обновление топика
export const updateTopic = createAsyncThunk<Topic, { id: string; updatedTopic: Partial<Topic>; token: string }>(
    "wiki/updateTopic",
...
// Удаление топика
export const deleteTopic = createAsyncThunk<string, { id: string; token: string }>(
    "wiki/deleteTopic",
    async ({ id, token }, { rejectWithValue }) => {
      try {
...
export const getThemesByTopic = createAsyncThunk<{ topicId: string; themes: Theme[] }, { topicId: string; token: string }>(
    "wiki/getThemesByTopic",
...
export const createTheme = createAsyncThunk<
  Theme,
  { topicId: string; newTheme: Omit<Theme, "id" | "createdAt" | "updatedAt">; token: string }
>(
  "wiki/createTheme",
...
export const getThemeById = createAsyncThunk<Theme, { themeId: string; token: string }>(
    "wiki/getThemeById",
    async ({ themeId, token }, { rejectWithValue }) => {
      try {
...
const wikiSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(getAllTopics.fulfilled, (state, action: PayloadAction<Topic[]>) => {
      ...
      // --- Темы ---
      .addCase(getThemesByTopic.fulfilled, (state, action: PayloadAction<{ topicId: string; themes: Theme[] }>) => {
      ...
      .addCase(updateTheme.fulfilled, (state, action: PayloadAction<Theme>) => {
        state.topics.forEach((topic) => {
          ...
          const index = topic.themes.findIndex((theme) => theme.id === action.payload.id);
          if (index !== -1) {
            topic.themes[index] = action.payload;
          }
        });
      })
      .addCase(deleteTheme.fulfilled, (state, action: PayloadAction<{ topicId: string; themeId: string }>) => {
        const topic = state.topics.find((t) => t.id === action.payload.topicId);
        if (topic?.themes) { 
          topic.themes = topic.themes.filter((theme) => theme.id !== action.payload.themeId);
        }
      });      
  },
});
