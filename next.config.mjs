...
const nextConfig = {
  images: {
    domains: process.env.NEXT_PUBLIC_ENV === "production" ? [] : ["localhost"], // разрешаем загрузку картинок с localhost
    remotePatterns: [
      {
        protocol: "http",
        hostname: process.env.NEXT_IMAGE_HOST || "tutorio-express",
        port: process.env.NEXT_PUBLIC_PORT, // тот же порт, что проброшен в docker-compose
        pathname: "/uploads/**", // доступ к картинкам в uploads
      },
    ],
  },
